# ARM halfword data transfer (immediate offset) (included by gba.nim)
...
proc arm_halfword_data_transfer_immediate*(cpu: CPU; instr: uint32) =
  ...
  let offset_high = bits_range(instr, 8, 11)
  let sh          = int(bits_range(instr, 5, 6))
  let offset_low  = bits_range(instr, 0, 3)
  var address     = cpu.r[rn]
  let offset      = (offset_high shl 4) or offset_low
  if pre_address:
    if add: address += offset else: address -= offset
  case sh
  of 0b00:
    raise newException(Exception, "HalfwordDataTransferImm swp " & hex_str(instr))
  of 0b01:  # ldrh/strh
    if load:
  ...
  of 0b11:  # ldrsh
    ...
    raise newException(Exception, "Invalid halfword data transfer imm op: " & $sh)
  if not pre_address:
    if add: address += offset else: address -= offset
