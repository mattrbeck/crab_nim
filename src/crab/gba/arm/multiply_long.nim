...
proc arm_multiply_long*(cpu: CPU; instr: uint32) =
  ...
  var res: int64 =
    if signed:
      int64(cast[int32](cpu.r[rm])) * int64(cast[int32](cpu.r[rs]))
    else:
      int64(uint64(cpu.r[rm]) * uint64(cpu.r[rs]))
  if accumulate:
    res += int64((uint64(cpu.r[rdhi]) shl 32) or uint64(cpu.r[rdlo]))
  discard cpu.set_reg(rdhi, uint32(uint64(res) shr 32))
